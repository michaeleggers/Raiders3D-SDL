...
TileMap::TileMap(std::string const& inputfile, int width, int height, Sprite* sprite)
    : columns_(width),
      rows_(height),
      sprite_(sprite)
{
...
void TileMap::draw(SDL_Renderer* renderer, int currentFrame)
{
    ...
    for ( int col = 0; col < columns_; ++col )
    {
        for ( int row = 0; row < rows_; ++row )
        {
            ...
            if ( tile == '#' )
            {
                SDL_RenderCopyEx(renderer, sprite_->getTexture_(), &frameRect, &destination, 0, NULL, SDL_FLIP_NONE);
            }
            if ( tile == '0' )
            {
                SDL_RenderCopyEx(renderer, sprite_->getTexture_(), &frameRect, &destination, 0, NULL, SDL_FLIP_NONE);
