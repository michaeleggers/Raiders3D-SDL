...
bbq::SpriteSheet::SpriteSheet(SDL_Renderer* renderer, const char* file, Uint32 key)
{
    ...
    if ( !surface )
    {
        std::cerr << "could not create surface from file " << std::string(file) << " ERROR: " << SDL_GetError()
                  << std::endl;
    ...
    if ( key )
    {
        ...
        int   success = SDL_SetColorKey(surface, SDL_TRUE, SDL_MapRGB(surface->format, r, g, b));
