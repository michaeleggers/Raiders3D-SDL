...
Sprite::Sprite(SpriteSheet* spriteSheet, int width, int height, Uint32 start_x, Uint32 start_y, Uint8 frames)
    : width_(width),
      height_(height)
{
    ...
    for ( int index = 0; index < frames_; ++index )
    {
        SDL_Rect frame = {
            ...
            int((((width * index) / spriteSheet_->width_) * height) + start_y), // top left y
            width,                                                              // x + width
            height                                                              // y + height
        };
