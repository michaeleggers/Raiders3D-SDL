...
Sprite::Sprite(SpriteSheet *spriteSheet, int width, int height, Uint32 start_x,
               Uint32 start_y, Uint8 frames)
  ...
  for (int index = 0; index < frames_; ++index) {
    SDL_Rect frame = {
        int((start_x + index * width) % spriteSheet_->width_), // top left x
        int((((width * index) / spriteSheet_->width_) * height) +
            start_y), // top left y
        width,        // x + width
        height        // y + height
    };
    frameRegions_.push_back(frame);
    std::cout << "frame " << index << ", x: " << frame.x << ", y: " << frame.y
              << std::endl;
  }
...
SDL_Texture *Sprite::getTexture_() { return spriteSheet_->texture_; }
...
Uint8 Sprite::frameCnt_() { return frames_; }
